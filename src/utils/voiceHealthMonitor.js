const { syncPlaytimeSession, stopPlaytimeSession } = require("./playtimeTracker");
            ...
            if (isIdle) {
                await this.client.rest
                    .put(`/channels/${player.voiceId}/voice-status`, {
                        body: { status: `use **${this.client.prefix}play** to add songs` },
                    })
                    .catch(() => null);
            }
                ...
                if (!player.paused) {
                    ...
                    if (lastPos !== undefined && lastPos === currentPos && currentPos > 0) {
                        ...
                        if (stalledDuration > this.CHECK_INTERVAL * 0.8) {
                            ...
                            await this.refreshConnection(player, guild, voiceChannel);
                            stopPlaytimeSession(this.client, player);
                            player.pause(true);
                            setTimeout(() => {
                                player.pause(false);
                                syncPlaytimeSession(this.client, player);
                            }, 1000);
                        }
                    } else {
