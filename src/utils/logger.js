...
class Logger {
  ...
  static log(content, type = "log") {
    ...
    switch (type) {
      ...
      case "warn": {
        const color = `${this.getColorCode("yellow")}[${date}]${reset}`;
        const typeColor = `${this.getColorCode("black")}${this.getColorCode(
          "bgYellow",
        )} ${type.toUpperCase()} ${reset}`;
        console.log(`${color}: ${typeColor} ${content}`);
      ...
      case "error": {
        ...
        const typeColor = `${this.getColorCode("white")}${this.getColorCode(
          "bgRed",
        )} ${type.toUpperCase()} ${reset}`;
        console.log(`${color}: ${typeColor} ${content}`);
        break;
      ...
      case "cmd":
      case "event": {
        const color = `${this.getColorCode("cyan")}[${date}]${reset}`;
        const typeColor = `${this.getColorCode("black")}${this.getColorCode(
          "bgCyan",
        )} ${type.toUpperCase()} ${reset}`;
        console.log(`${color}: ${typeColor} ${content}`);
      ...
      default:
        throw new TypeError(
          "Logger type must be either warn, debug, info, log, ready, cmd, event or error.",
        );
    }
  }
}
