...
const fallbackEngines = ["ytsearch", "ytmsearch", "scsearch", "spsearch", "amsearch", "dzsearch"];
...
module.exports = function loadPlayerManager(client) {
  ...
  const manager = new Kazagumo(
    {
      ...
      plugins: spotifyId && spotifySecret ? [
        new Spotify({
          clientId: spotifyId,
  ...
  manager.search = async function (query, options = {}) {
    let node = getAvailableNode(this);
    if (!node) {
      await ensureNodeConnection(client, { maxWaitTime: options.nodeWaitTime ?? 3000 });
      node = getAvailableNode(this);
    }
    ...
    if (!isUrl) {
      const preferredEngine = options.engine || this.KazagumoOptions?.defaultSearchEngine;
      const searchEngineList = [...new Set([preferredEngine, ...fallbackEngines].filter(Boolean))];
      ...
      for (const engine of searchEngineList) {
        ...
        if (searchRes && searchRes.loadType !== 'EMPTY' && searchRes.loadType !== 'ERROR' && searchRes.loadType !== 'NO_MATCHES') {
          const result = processSearchResult(searchRes, options.requester);
