...
module.exports = {
  ...
  run: async (client, message) => {
    ...
    if (message.content.match(mention)) {
      ...
      const greetDisplay = new TextDisplayBuilder()
        .setContent(
          ...
          `**${client.emoji.info} Type \`${prefix}play <song>\` to play music.**`
        );
    ...
    if (command.userPerms && !client.owners.includes(message.author.id) && !message.member.permissions.has(PermissionsBitField.resolve(command.userPerms || []))) {
      const permDisplay = new TextDisplayBuilder()
