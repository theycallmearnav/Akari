...
const { syncPlaytimeSession, stopPlaytimeSession } = require("../../utils/playtimeTracker");
...
module.exports = {
  ...
  run: async (client, oldState, newState) => {
    ...
    if (
      player &&
      oldState.id !== botId &&
      (oldState.channelId === player.voiceId || newState.channelId === player.voiceId)
    ) {
      syncPlaytimeSession(client, player);
    }

    const currentChannel = newState.channel?.members.has(botId)
      ? newState.channel
